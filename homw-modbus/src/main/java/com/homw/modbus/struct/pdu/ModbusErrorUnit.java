 ...
 *
 */
public class ModbusErrorUnit extends ModbusProtoUnit {
	/*
	 ...
	 * to newer devices, and was not implemented in the unit selected. It could also
	 * indicate that the server (or slave) is in the wrong state to process a
	 * request of this type, for example because it is unconfigured and is being
	 ...
	 * address of 96 and a quantity of registers of 5, then this request will fail
	 * with Exception Code 0x02 “Illegal Data Address” since it attempts to operate
	 * on registers 96, 97, 98, 99 and 100, and there is no register with address
	 * 100.
	 *
	 ...
	 * time will be required to do so. This response is returned to prevent a
	 ...
	 * check. The server (or slave) attempted to read record file, but detected a
	 * parity error in the memory. The client (or master) can retry the request, but
	 ...
	 * output port for processing the request. Usually means that the gateway is
	 ...
	 * obtained from the target device. Usually means that the device is not present
	 * on the network.
	 */
	...
	public ModbusErrorUnit(short funcCode) {
		super(funcCode);
	}

	public ModbusErrorUnit(short functionCode, short exceptionCode) {
		super(functionCode);
	...
	public byte[] getUnitBytes() {
		return new byte[] { (byte) funcCode, (byte) exceptionCode };
	}
