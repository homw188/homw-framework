...
public class KedeElecOpenUtil {
	 ...
	 * @return json字符串：lj ：累计用电; sy:表示剩余电量,cs
	 *         表示充值次数({"data":{"lj":"000053.96","sy":"-000000.00","cs":"0000"})
	 */
	public static String ztcx(String ip, int port, String addr, int waiting) {
		try {
			...
			try {
				...
				readStr = nettyClient.send(strr, waiting);
			} finally {
				nettyClient.close();
			}
			logger.info("back data: " + readStr);

			return KedeDataProtocolUtil.ZtcxData(readStr);
		} catch (Exception e) {
			return null;
		}
	}
	 ...
	 */
	public static String eleAction(String ip, int port, String addr, int TD, int waiting) {
		try {
			Date da = new Date();
			...
			return KedeDataProtocolUtil.Set94(readStr);
		} catch (Exception e) {
			return null;
		}
	}
	...
	public static String eleBuy(String ip, int port, String addr, int waiting, int Ele_C, int Cs, int Fx) {
		try {
			...
			return KedeDataProtocolUtil.Set83(readStr);
		} catch (Exception e) {
			return null;
		}
	}
